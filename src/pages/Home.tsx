...
export default function Home() {
	...
	useEffect(() => {
		const accepted = document.cookie
			.split('; ')
			.some((row) => row.startsWith('privacy-accepted='));
			...
			const startTyping = () => {
				if (contentQueueRef.current.length > 0) {
					...
					const charsToAdd = Math.max(
						1,
						Math.floor(contentQueueRef.current.length / 20)
					);
					const chunk = contentQueueRef.current.slice(0, charsToAdd);
					contentQueueRef.current =
						contentQueueRef.current.slice(charsToAdd);
	...
	return (
		<div
			className={`flex h-[100dvh] w-full overflow-hidden font-sans transition-colors duration-300 ${
				isLightMode
		...
		>
			<div className="flex-1 flex flex-col relative w-full h-full">
				<header
					className={`px-4 py-3 flex items-center justify-between fixed top-0 left-0 right-0 z-30 transition-all duration-300 bg-gradient-to-b ${
						isLightMode
							? 'from-[#f5f5f5] via-[#f5f5f5]/95'
							: 'from-[#161616] via-[#161616]/95'
					} to-transparent`}
				>
					...
					<div className="flex-1 flex justify-center">
						{messages.length > 0 && (
							...
							>
								<Trash2 size={14} />
								<span className="hidden md:inline tracking-wider font-medium">
									{t('chat.clear_chat')}
								</span>
							</button>
						)}
				...
				>
					<div className="max-w-3xl mx-auto py-8 pt-24 pb-48">
						{messages.length === 0 ? (
							<div
								className={`h-full flex flex-col items-center justify-center mt-20 md:mt-32 opacity-50 px-4 text-center transition-colors duration-300 ${
							...
							>
								...
								<p
									className={`text-xs md:text-sm mt-2 max-w-md transition-colors duration-300 ${isLightMode ? 'text-gray-500' : 'text-gray-400'}`}
								>
									{t('chat.privacy_warning')}
								</p>
							...
							messages.map((msg, idx) => {
								...
								const isPrivacy =
									msg.role === 'assistant' &&
									privacyRegex.test(msg.content.trim());
								const isAbort =
									msg.role === 'assistant' &&
									abortRegex.test(msg.content.trim());
								const isOffTopic =
									msg.role === 'assistant' &&
									offTopicRegex.test(msg.content.trim());
								const hasTable =
									msg.role === 'assistant' &&
									/\|.+\|/.test(msg.content) &&
								...
								return (
									<div
										key={idx}
										className={`mb-6 md:mb-8 flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}
									>
										<div
											className={`px-3 py-2 md:px-4 md:py-2.5 rounded-[20px] md:rounded-[24px] transition-colors duration-300 ${
												hasTable
													? 'max-w-full w-full'
										...
										>
											{msg.role === 'assistant' && (
												<div
													className={`flex items-center gap-2 mb-1.5 text-xs transition-colors duration-300 ${
														isAbort
															? isLightMode
																? 'text-red-700 opacity-100 font-bold'
																: 'text-red-400 opacity-100 font-bold'
															: isPrivacy
																? isLightMode
																	? 'text-yellow-700 opacity-100 font-bold'
																	: 'text-yellow-400 opacity-100 font-bold'
																: isOffTopic
																	? isLightMode
																		? 'text-blue-700 opacity-100 font-bold'
				...
				<div
					className={`p-4 pb-4 transition-colors duration-300 fixed bottom-0 left-0 right-0 z-20 ${isLightMode ? 'bg-gradient-to-t from-[#f5f5f5] via-[#f5f5f5] to-transparent' : 'bg-gradient-to-t from-[#161616] via-[#161616] to-transparent'}`}
				>
					<div className="max-w-3xl mx-auto relative group px-4 md:px-0">
						<div
							className={`relative flex items-start w-full rounded-[24px] md:rounded-[30px] border transition-all duration-300 p-1.5 md:p-2 pr-2 md:pr-2 shadow-xl ${
								isLightMode
						...
						>
							...
							>
								{models.length === 0 ? (
									<div
										className={`flex items-center h-9 md:h-10 px-2 md:px-3 rounded-full cursor-wait opacity-50 ${
											isLightMode
												? 'hover:bg-black/5'
												: 'hover:bg-white/5'
										}`}
									...
									<div className="relative">
										...
										>
											<Boxes
												size={18}
												className={`transition-colors duration-300 ${isLightMode ? 'text-gray-600' : 'text-gray-200'}`}
											/>
										</button>

										{isModelOpen && (
											<div
												className={`absolute bottom-full left-0 mb-3 w-64 border rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200 z-50 transition-colors ${
													isLightMode
											...
											>
												<div
													className={`p-2 border-b ${isLightMode ? 'border-black/5' : 'border-white/5'}`}
												>
													<span className="px-3 py-1 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
														{t('chat.models') ||
															'Models'}
													</span>
												</div>
													...
													{models.map((model) => (
														<button
															...
															className={`w-full flex items-center gap-3 px-4 py-3 text-sm transition-colors ${
																isLightMode
																	? selectedModel ===
																		model.id
																		? 'text-gray-900 font-bold bg-black/5'
														...
														>
															...
															{selectedModel ===
																...
																<Bot
																	size={16}
																	className={`ml-auto ${isLightMode ? 'text-gray-900' : 'text-white'}`}
																/>
															)}
														</button>
													))}
							...
							<textarea
								...
								placeholder={
									isAccepted
										? t('chat.placeholder')
										: t('chat.placeholder_accept_first')
								}
								rows={1}
								maxLength={1000}
							...
							{input.length >= 500 && (
								<div
									className={`absolute -top-6 right-4 text-[10px] font-mono transition-colors ${
										input.length >= 1000
											? 'text-red-500'
											: input.length > 700
												? 'text-yellow-500'
								...
								<button
									...
									className={`flex items-center justify-center w-9 h-9 md:w-10 md:h-10 rounded-full transition-all duration-300 flex-shrink-0 ${
										input.trim() && !isLoading && isAccepted
											...
											: isLightMode
												? 'bg-black/5 text-gray-400 opacity-50'
			...
			{!isAccepted && (
				...
				>
					<div
						className={`max-w-md w-full rounded-3xl p-5 md:p-7 border shadow-2xl animate-in fade-in zoom-in duration-300 ${
							isLightMode
								? 'bg-white border-black/10'
					...
					>
						<div className="flex items-center justify-between mb-4">
							<h3
								className={`text-xl md:text-2xl font-bold ${isLightMode ? 'text-gray-900' : 'text-white'}`}
							>
								{t('privacy.title')}
							</h3>
						...
						<div
							className={`text-sm md:text-[15px] space-y-4 mb-8 leading-relaxed ${isLightMode ? 'text-gray-600' : 'text-gray-400'}`}
						>
							<p
								dangerouslySetInnerHTML={{
									__html: t('privacy.paragraph1')
								}}
							/>
							<p
								dangerouslySetInnerHTML={{
									__html: t('privacy.paragraph2')
								}}
							/>
							<p
								className={`text-[11px] md:text-xs italic ${isLightMode ? 'text-gray-500' : 'text-gray-500'}`}
							>
			...
			{externalLink.isOpen && (
				...
				>
					<div
						className={`max-w-lg w-full rounded-[24px] p-6 md:p-7 border shadow-2xl animate-in fade-in zoom-in duration-200 ${
							isLightMode
								? 'bg-white border-black/10'
								: 'bg-[#212121] border-white/10'
						}`}
					>
						<div className="flex items-center justify-between mb-4">
							<h3
								className={`text-lg font-bold ${isLightMode ? 'text-gray-900' : 'text-white'}`}
							>
								{t('external_link.title')}
							</h3>
							<LanguageSelector align="right" />
						</div>

						<p
							className={`text-sm leading-relaxed mb-4 ${isLightMode ? 'text-gray-600' : 'text-gray-400'}`}
						>
						...
						>
							...
							<span
								className={`text-xs break-all font-mono opacity-80 ${isLightMode ? 'text-gray-600' : 'text-gray-400'}`}
							>
								{externalLink.url}
							</span>
						</div>
